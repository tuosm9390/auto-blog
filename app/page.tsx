...
export default async function LandingPage() {
  ...
  const reviews = [
    ...
    { quote: "커스타마이징의 유연성이 돋보입니다. AI가 써준 글이 100% 맘에 들지는 않을 때 마크다운으로 수정하고 배포할 수 있어서 좋습니다.", role: "시니어 아키텍트" },
    { quote: "검색엔진 트래픽이 눈에 띄게 늘었습니다. 시맨틱 마크업 처리가 잘 되어 있어서 그런지 오가닉 유입이 이전 세팅보다 확연히 증가했습니다.", role: "프리랜서 개발자" },
    { quote: "이제 '블로그 써야지' 하는 부채감에서 해방되었습니다. 코딩만 하면 블로그는 알아서 업데이트된다는 경험 자체가 혁명입니다.", role: "iOS 개발자" },
    { quote: "다크모드 타이포그래피가 정말 예술입니다. 첫화면 진입하자마자 느껴지는 시각적 완성도가 압도적입니다.", role: "UX/UI 엔지니어" },
    { quote: "지원자 평가 시간이 절반으로 줄었습니다. 비개발자도 역량을 파악하기 너무 쉽게 비즈니스 임팩트가 요약되어 있습니다.", role: "IT 기업 테크 리쿠르터" },
    { quote: "프리랜서 계약 전 신뢰도를 100% 채워줍니다. 디자인에서 오는 프리미엄 느낌 때문에 기술력에 대한 의심 없이 바로 미팅을 제안했습니다.", role: "스타트업 대표" },
    { quote: "투자 관점에서 창업자의 실행력을 엿보기 좋습니다. 매일 얼마나 많은 문제를 밀도 있게 해결하고 있는지 하이레벨 요약본으로 볼 수 있습니다.", role: "벤처 캐피탈리스트" },
    { quote: "가독성이 뛰어나 모바일로 슥슥 읽기 좋습니다. 긴 문장이 알아서 끊어져 있고 핵심만 렌더링되어 있어서 눈이 피로하지 않네요.", role: "시니어 테크니컬 디렉터" },
    { quote: "팀 내 개발 문화 가이드로 써도 될 만큼 요약이 훌륭합니다. 신입 개발자들의 컨텍스트 파악이 훨씬 수월할 것 같습니다.", role: "개발 팀장" },
    { quote: "연락하고 싶게 만드는 CTA 폼이 인상적입니다. 제안하는 과정이 매우 매끄럽게 설계되어 있습니다.", role: "헤드헌터" }
  ];
    ...
    <div className="max-w-6xl mx-auto px-4 animate-fade-in-up">
      ...
      <section className="flex flex-col items-center text-center py-24 md:py-32">
        ...
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-display font-bold tracking-tight leading-tight mb-6">
          코딩만 하세요.<br />
          기술 블로그는 <span className="text-text-secondary">AI</span>가 완성합니다.
        </h1>
        <p className="max-w-2xl text-text-secondary text-lg md:text-xl mb-10 leading-relaxed font-body">
          GitHub 커밋을 분석하여 자동으로 기술 블로그 포스트를 생성합니다.<br />
          개발에만 집중하세요 — 나머지는 AI가 처리합니다.
        </p>
        <div className="flex gap-4 flex-wrap justify-center">
          <Link href={session ? "/posts" : "/login"} className="px-8 py-3 bg-accent text-black font-semibold rounded-lg hover:bg-accent-hover transition-colors">
            {session ? "포스트 보기" : "시작하기"}
          </Link>
          <Link href="/how-it-works" className="px-8 py-3 border border-border-strong rounded-lg text-text-secondary hover:text-text-primary hover:border-text-primary transition-all">
            작동 방식
          </Link>
        </div>
      </section>
      ...
      <section className="py-16 border-t border-border-subtle">
        <h2 className="text-2xl md:text-3xl font-display font-semibold text-center mb-12">왜 AutoBlog인가?</h2>
        <div className="grid md:grid-cols-3 gap-6">
          {[
            { icon: "⚡", title: "완전 자동화", desc: "커밋이 감지되면 AI가 알아서 분석하고 글을 작성합니다." },
            { icon: "🧠", title: "딥 코드 분석", desc: "단순 커밋 로그가 아닌, 코드 변경의 의도와 맥락을 파악합니다." },
            { icon: "✍️", title: "전문가 톤", desc: "시니어 개발자가 회고하듯 깊이 있는 기술 글을 생성합니다." },
          ].map((f) => (
      ...
      <section className="py-24 border-t border-border-subtle overflow-hidden">
        <div className="text-center mb-16 px-4">
          ...
          <p className="text-text-secondary max-w-xl mx-auto">
            매주 주말을 희생하지 마세요. 현업 개발자와 채용 담당자가 경험한 오토블로그의 마법.
          </p>
        </div>
          ...
          <div className="flex select-none gap-6 animate-marquee whitespace-nowrap">
            {[...reviews.slice(0, 10), ...reviews.slice(0, 10)].map((review, idx) => (
              <div key={idx} className="w-[400px] flex-shrink-0 border border-border-subtle rounded-2xl p-6 bg-surface/50 hover:bg-surface hover:border-border-strong transition-all duration-300">
                <p className="text-text-primary text-sm leading-relaxed mb-4 whitespace-normal line-clamp-3">"{review.quote}"</p>
      ...
      <section className="py-16 border-t border-border-subtle text-center">
        <h2 className="text-2xl md:text-3xl font-display font-semibold mb-4">지금 바로 시작하세요</h2>
        <p className="text-text-secondary mb-8 max-w-xl mx-auto whitespace-pre-line">
          {session ? "버튼을 클릭하여 포스트를 확인하고 관리해보세요.\n당신의 코드가 가치 있는 아티클로 변하는 순간을 경험하세요." : "GitHub 계정으로 로그인하면 즉시 사용할 수 있습니다.\n지금 바로 첫 포스팅을 만들어보세요."}
        </p>
