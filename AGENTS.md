...
핵심 스택은 Next.js App Router, React 19, Tailwind v4, NextAuth v5, Supabase, Gemini SDK, PortOne V2다.
...
├── app/api/             # 인증된 API, 관리자 API, PortOne 웹훅
├── components/          # 공유 UI와 프로젝트 문서 workspace 컴포넌트
├── lib/                 # 프로젝트, 문서, AI, 결제, GitHub 도메인 서비스
├── tests/               # Vitest 기능 테스트
...
| 결제와 구독 | `lib/portone-billing.ts`, `lib/subscription.ts`, `app/api/portone/**`, `app/api/webhooks/portone/**` | Stripe는 레거시 스텁 |
...
| `handleWebhookEvent` | billing service | `lib/portone-billing.ts` | PortOne 웹훅 처리와 멱등성 보장 |
...
- 신규 결제 코드에서 Stripe를 사용하지 않는다. `lib/billing.ts`와 `lib/stripe.ts`는 레거시 스텁이다.
...
- CSP에 와일드카드 `*`를 추가하지 않는다. PortOne/TossPayments 도메인은 `next.config.ts`의 명시 목록을 유지한다.
...
- Vercel cron은 `/api/cron/billing`을 매일 `01:00 UTC`에 호출한다.
- `README.md`와 일부 오래된 문서에는 Stripe 표기가 남아 있을 수 있다. 현재 결제 source of truth는 PortOne V2 규칙과 `lib/portone-billing.ts`다.
